 ...
 */
async function changeReleaseInJson(jsonPath, release) {
    try {
 ...
 *          and the process will exit with code 1.
 */
async function performGitCommands(RELEASE_VERSION, branch) {
    try {
...
export async function release() {
    while (true) {
        ...
        if (!hasUpdatedChangelog) {
            console.log(chalk.yellow(`Changelog section for ${RELEASE_VERSION} not found. Please update the changelog.`));
            return;
        }
        await changeReleaseInJson(PACKAGE_PATH, RELEASE_VERSION);
        await changeReleaseInJson(MANIFEST_PATH, RELEASE_VERSION);
        console.log(chalk.green(`Version updated to ${RELEASE_VERSION} in package.json and manifest.json`));
        ...
        const currentBranch = execSync('git rev-parse --abbrev-ref HEAD', {
            ...
            .trim();
        while (true) {
            console.log(`You are currently on branch ${currentBranch}. The new tag will be created on remote ${currentBranch}.`);
        ...
        console.log(chalk.green(`Release ${RELEASE_VERSION} has been successfully created and pushed.`));
        process.exit(0);
    }
}
