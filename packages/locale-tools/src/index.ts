...
async function checkAllLocalesAction(): Promise<void> {
    ...
    for (const locale of locales) {
        ...
        try {
            ...
            const untranslatedKeys = [...enKeys].filter(
                (key) =>
                    flatKeys.has(key) &&
                    flatData[key] === enFlat[key] &&
                    (typeof flatData[key] === 'string'
                        ? flatData[key].trim() !== ''
                        : Array.isArray(flatData[key]) &&
                          flatData[key].length > 0)
            );
            ...
            if (untranslatedKeys.length > 0) {
                console.log(`🔄 Untranslated values (same as English):`);
                untranslatedKeys.slice(0, 5).forEach((key) => {
                    console.log(`  - ${key}: "${enFlat[key]}"`);
                });
                if (untranslatedKeys.length > 5) {
                    console.log(
                        `  ... and ${untranslatedKeys.length - 5} more`
                    );
                }
            }
    ...
    if (results.length > 0) {
            ...
            .forEach((result) => {
                ...
                const untranslatedNote =
                    result.untranslated.length > 0
                        ? ` (${result.untranslated.length} untranslated)`
                        : '';

                console.log(
                    `  ${result.locale}:${padding}${result.completed}/${totalKeys} (${result.percentage}%) ${statusIcon}${untranslatedNote}`
                );
            });
        ...
        const totalUntranslated = results.reduce(
            (sum, r) => sum + r.untranslated.length,
            0
        );
        if (totalUntranslated > 0) {
            console.log(`🔄 Total untranslated values: ${totalUntranslated}`);
        }
    }
}
